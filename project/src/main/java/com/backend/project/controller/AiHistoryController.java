...
public class AiHistoryController {
    ...
    public ResponseEntity<?> createAiHistory(@RequestParam String email, @RequestBody AiHistory aiHistory) {
        try {
            aiHistoryService.createHistory(email, aiHistory.getRoleType(), aiHistory.getRequestContent(), aiHistory.getResponseResult(), aiHistory.getGenerateDate(), aiHistory.getAdditionalInfo());
